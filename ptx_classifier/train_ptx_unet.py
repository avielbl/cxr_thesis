...
def prep_data():
    ...
    train_masks_arr = np.zeros((nb_train, 1, im_size, im_size))
    val_imgs_arr = np.zeros((nb_val, 1, im_size, im_size))
    val_masks_arr = np.zeros((nb_val, 1, im_size, im_size))
    for i in range(nb_train):
        train_imgs_arr[i] = train_data_lst[i]['img']
        train_masks_arr[i] = train_data_lst[i]['ptx_mask']
    for i in range(nb_val):
        val_imgs_arr[i] = val_data_lst[i]['img']
        val_masks_arr[i] = val_data_lst[i]['ptx_mask']
...
def train_model(db):
    ...
    model = get_unet(im_size, optim_fun=optim_fun)
    model_file_name = 'ptx_model_unet' + time.strftime("%H_%M_%d_%m_%Y") + '.hdf5'
    ...
    model.fit(db[0], db[1], batch_size=4, epochs=100,
